...
class CalledPartyTracing(SimpleAXLAPI):
    ...
    supported_methods = ["add", "list", "remove", "model"]
...
# JRE added - not tested yet
# choice elements are different in that nothing is required
# what makes a unique element for a get?  To test
class InfrastructureDevice(SimpleAXLAPI):
    _factory_descriptor = "infrastructure_device"
    supported_methods = ["add", "get", "update", "list", "remove", "model"]
...
class MraServiceDomain(SimpleAXLAPI):
    _factory_descriptor = "mra_service_domain"
    supported_methods = ['model', 'create', 'add', 'update', 'get', 'list', 'remove', 'add_update']
    # confirm this list matches standard listing  and remove this line if possible

    def add(self, name, serviceDomains, **kwargs):
        add_kwargs = flatten_signature_kwargs(self.add, locals())
        return super().add(**add_kwargs)
...
class RemoteCluster(SimpleAXLAPI):
    ...
    supported_methods = ["model", "create", "add", "get", "update", "list", "remove", "do_update", "add_update"]
